# This program returns the hospital with the rank specified as input variable 'num' 
# when the outcomes are sorted in ascending order of rate of outcome for a given state
...
rankhospital <- function(state,outcome,num="best") {
  ...
  # Set the value of num for various scenarios
  if(num=="best") {
    num=1
  } else if (num=="worst") {
  num=length(state_df$Outcome_Col)  
  } else if (num>length(state_df$Outcome_Col)) {
    return("NA")
  } 
  print(num)
  
  # Return the hospital with the row value=num
  return(as.character(state_df[num,1]))
